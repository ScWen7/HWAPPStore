...
//                        EventBus.getDefault().post(new ClearCacheEvent());
    ...
    public void clearImageMemoryCache(Context context) {
        try {
            if (Looper.myLooper() == Looper.getMainLooper()) { //清除缓存只能在主线程执行
    ...
    private static String getFormatSize(double size) {
        ...
        if (gigaByte < 1) {
            ...
            BigDecimal bigDecimal = result2.setScale(2, BigDecimal.ROUND_HALF_UP);
           if(bigDecimal.compareTo(new BigDecimal(40.00))==1) {
                bigDecimal = new BigDecimal(40.00);
           }
